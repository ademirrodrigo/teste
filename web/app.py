...
# Garante que o pacote backend ``app`` seja resolvido corretamente quando este
# arquivo for executado pelo Streamlit como ``app`` (nome do arquivo).
FRONTEND_FILE = Path(__file__).resolve()
PROJECT_ROOT = FRONTEND_FILE.parents[1]
...
if existing_app_module and getattr(existing_app_module, "__file__", None):
    if Path(existing_app_module.__file__).resolve() == FRONTEND_FILE:
        del sys.modules["app"]
...
def pagina_login() -> None:
    st.set_page_config(page_title="Coletor Fiscal v3.2", page_icon="🧾", layout="centered")
