...
# O Streamlit executa este arquivo como "app" dentro da pasta "web". Para
# garantir que o pacote backend ``app`` (diretório ``app/``) seja priorizado nas
# importações, colocamos o diretório raiz do projeto na frente do ``sys.path`` e
# removemos temporariamente a pasta deste frontend.
PROJETO_RAIZ = Path(__file__).resolve().parents[1]
FRONTEND_DIR = Path(__file__).resolve().parent

projeto_raiz_str = str(PROJETO_RAIZ)
frontend_dir_str = str(FRONTEND_DIR)

if frontend_dir_str in sys.path:
    sys.path.remove(frontend_dir_str)
if projeto_raiz_str not in sys.path:
    sys.path.insert(0, projeto_raiz_str)
