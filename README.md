...
Antes de executar os scripts assegure-se de que o Python 3.11+ está instalado e disponível no `PATH`. Os utilitários abaixo detectam automaticamente `py.exe` ou `python.exe` e exibem mensagens claras em caso de falhas.

1. Abra **PowerShell** ou **Prompt de Comando** e navegue até a pasta do projeto:
   ```powershell
   cd C:\SISTEMAS\ColetorFiscal\teste
   ```
2. Execute `install.bat` para criar a `.venv`, atualizar o `pip` e instalar as dependências (com tratamento de erros e instruções adicionais):
   ```powershell
   .\install.bat
   ```
3. Ajuste o arquivo `.env` conforme necessário.
4. Inicialize o painel com `start.bat`, que reutiliza a mesma `.venv` e expõe a porta configurada nas variáveis de ambiente:
   ```powershell
   .\start.bat
   ```
5. Para validar o ambiente a qualquer momento, rode `verify.bat`, que executa `pip check` e compila o código para detectar problemas precocemente:
   ```powershell
   .\verify.bat
   ```
6. Caso precise refazer todo o ambiente virtual, utilize `force_install.bat`, que remove a `.venv` anterior e reinstala tudo com `pip --no-cache-dir`:
   ```powershell
   .\force_install.bat
   ```

Os scripts aceitam as variáveis `PORTA` e `ENDERECO`. Defina-as antes de chamar o `start.bat` se precisar personalizar o binding:

```powershell
$env:PORTA = 8502
$env:ENDERECO = "127.0.0.1"
.\start.bat
```
